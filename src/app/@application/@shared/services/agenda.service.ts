...
export class AgendaService {
  ...
  getUserAgendas(id: number = this._authService.user.id): Observable<Array<Agenda>> {
    return this._http.get<Array<Agenda>>(`${ this._baseUrl }/user/${ id.toString(10) }`).pipe(
      map((agendas: Array<Agenda>) => agendas.map((agenda: Agenda) => new Agenda(agenda))),
    );
  }

  getUserSharedAgendas(id: number = this._authService.user.id): Observable<Array<Agenda>> {
    return this._http.get<Array<Agenda>>(`${ this._baseUrl }/shared/user/${ id.toString(10) }`).pipe(
      map((agendas: Array<Agenda>) => agendas.map((agenda: Agenda) => new Agenda(agenda))),
    );
  }
